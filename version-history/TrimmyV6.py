...
# --- MODIFICATION START: Status message clear delay ---
STATUS_MESSAGE_CLEAR_DELAY_MS = 5000 # 5 seconds
# --- MODIFICATION END ---
...
# --- MODIFICATION: Removed open_file_explorer as it's no longer used by post_trim_success ---
...
class VideoTrimmerApp(customtkinter.CTk):
    ...
    def __init__(self, initial_input_dir): # Changed parameter name
        ...
        # --- MODIFICATION START: Add status message clear job ---
    ...
    def update_video_selection_dropdown(self):
        ...
        if self.video_filenames:
            ...
            # --- MODIFICATION START: Clear status after successful video load only if it was a search message ---
            # self.update_status("") # Clear searching message
            current_status_text = self.status_label.cget("text")
            if "Searching for videos" in current_status_text:
                self.update_status("")
            # --- MODIFICATION END ---
        else:
    ...
    def load_video_data(self, video_path):
        ...
        # --- MODIFICATION START: Clear status after successful metadata load only if it was a loading message ---
        # self.update_status("") # Clear loading message
        current_status_text = self.status_label.cget("text")
        if "Loading metadata for" in current_status_text:
            self.update_status("")
        # --- MODIFICATION END ---
    ...
    def run_ffmpeg_trim(self, delete_original, temp_output_path_for_delete):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                # --- MODIFICATION START: Construct success message with path ---
                success_msg_base = f"Done! Trimmed video saved at: {os.path.normpath(final_output_path)}"
                # --- MODIFICATION END ---
                if delete_original:
                    self.update_status("Deleting original...", "blue"); time.sleep(0.5) # Keep this intermediate status
                    try:
                        print(f"Attempt delete: {input_filename}"); os.remove(input_filename); print(f"Delete OK: {input_filename}")
                        self.update_status("Renaming...", "blue"); time.sleep(0.5) # Keep this intermediate status
                        try:
                            ...
                            while os.path.exists(final_path_to_use): final_path_to_use = f"{base_final}_{counter}{ext_final}"; counter += 1 # This ensures the final rename target is unique
                            if final_path_to_use != final_output_path:
                                ...
                                # Update success message if path changed
                            ...
                            # --- MODIFICATION START: Update success message for delete case ---
                            full_success_msg = f"{success_msg_base}\nOriginal file permanently deleted."
                            self.update_status(full_success_msg, "green")
                    ...
                    # --- MODIFICATION START: Update success message for normal trim ---
                    self.update_status(success_msg_base, "green")
    ...
    def post_trim_success(self, output_filepath):
        ...
        # --- MODIFICATION START: Remove explorer opening and auto-close ---
        # open_file_explorer(output_filepath)
        # self.update_status("Done! Closing...", "green") # Status already set in run_ffmpeg_trim
        # --- MODIFICATION END ---

        # --- MODIFICATION START: Schedule status message clear and reset UI ---
        # Clear status message after a delay
        if self.status_message_clear_job:
            self.after_cancel(self.status_message_clear_job)
        self.status_message_clear_job = self.after(STATUS_MESSAGE_CLEAR_DELAY_MS, lambda: self.update_status(""))

        # Reset UI to an "initial-like" state by refreshing video list.
        # This will also re-enable controls appropriately.
        self.is_processing = False # Allow UI reset to fully enable components
        self.after(100, self.update_video_selection_dropdown) # Refresh video list, which reloads UI
        # --- MODIFICATION END ---


    def reset_ui_after_processing(self):
        """Resets UI elements after processing (typically after an error)."""
        self.is_processing = False
        # Always enable directory/video selectors
        self.location_combobox.configure(state="normal")
        # --- MODIFICATION START: Ensure video combobox state is correctly set based on video_filenames ---
        ...
        #
        # The logic below is more robust for resetting after an error, as self.video_path might be stale.
        # We effectively re-evaluate if UI should be enabled based on current video files.
        self.update_video_selection_dropdown() # This will refresh and set states correctly.
        # If update_video_selection_dropdown leads to no videos, disable_ui_components(True) will be called.
        # If videos are found, on_video_selected will be called, leading to disable_ui_components(False).
        # --- MODIFICATION END ---


    def update_status(self, message, color="gray"):
        """Updates the status label text and color."""
        # --- MODIFICATION START: Cancel previous clear job if a new message is set ---
        if self.status_message_clear_job:
            self.after_cancel(self.status_message_clear_job)
            self.status_message_clear_job = None
        # --- MODIFICATION END ---
    ...
    def on_closing(self):
        ...
        # --- MODIFICATION START: Cancel status clear job on closing ---
        if self.status_message_clear_job:
            self.after_cancel(self.status_message_clear_job)
        # --- MODIFICATION END ---
        cleanup_temp_files()
